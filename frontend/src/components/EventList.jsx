...
const mockFallbackEvents = [
  ...
  },
  {
    ...
    imageUrlAlt: 'Medical volunteers assisting donors at community health desk'
  },
  {
    _id: '4',
    id: 'evt-4',
    ...
    imageUrlAlt: 'Keynote speaker addressing an audience at conference'
  },
  {
    _id: '5',
    id: 'evt-5',
    ...
    imageUrlAlt: 'Folk dancers performing in traditional colorful attire'
  }
];
...
export function EventList({ onRegisterEvent, onSelectEvent }) {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      let url = search
        ? `/api/events/search?q=${encodeURIComponent(search.trim())}`
      ...
      if (res.ok) {
        ...
        setEvents(eventData.length > 0 ? eventData : mockFallbackEvents);
      } else {
        setEvents(mockFallbackEvents);
      }
    } catch (e) {
      console.warn('Error fetching events from API:', e.message);
      setEvents(mockFallbackEvents);
    } finally {
      setLoading(false);
    }
  ...
  const filteredEvents = events.filter(e => {
    ...
    if (search) {
      ...
      const locStr = typeof e.location === 'object' ? e.location.placeName : String(e.location || '');
      return (
    ...
    <Container maxWidth="lg" sx={{ py: 4 }} id="events">
      <Box sx={{ mb: 4, textAlign: 'center' }}>
        <Typography variant="h4" component="h1" sx={{ fontWeight: 800, color: '#0f172a', mb: 1 }}>
          {t('events.sectionTitle', 'Upcoming Community Events')}
        </Typography>
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: 'center', mb: 4, gap: 2 }}>
        <Tabs
          ...
          scrollButtons="auto"
        >
          <Tab label={t('allCategories', 'All Events')} value="all" sx={{ fontWeight: 700 }} />
          <Tab label={t('career', 'Career & Jobs')} value="career" sx={{ fontWeight: 700 }} />
          <Tab label={t('health', 'Health & Wellness')} value="health" sx={{ fontWeight: 700 }} />
          <Tab label={t('culture', 'Cultural')} value="culture" sx={{ fontWeight: 700 }} />
          <Tab label={t('workshop', 'Workshops')} value="workshop" sx={{ fontWeight: 700 }} />
        </Tabs>
      ...
      {loading ? (
        ...
        <Grid container spacing={3}>
          {filteredEvents.map(event => (
            <Grid item key={event._id || event.id || event.title} xs={12} sm={6} md={4}>
              <EventCard
                event={event}
